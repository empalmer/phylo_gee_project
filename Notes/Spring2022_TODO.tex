...
\begin{todolist}
  \item Run through with bug fixed. See what goes on. Write up results
  ...
  \item Clean up old code
  \item Think about how to do simulations, since we need to have something to check against.
  \item Save beta values again and look at changes.
  \item[\done] Convert initilization to one line list.
  \item Consider moving functions to new files
  \item Make gamma, lambda values arguments in the functions.
  \item Save final residuals in model.
  \item Figure out why flavo dataset stops in 2 iterations
  \item See if weird jump still happens. figure this out.
  \item Add model call to result list
  \item[\done] Create diagnoistics plot function 
  \begin{itemize}
    \item Patchwork outputs
  \end{itemize}
  \item Figure out a way to compare beta values
\end{todolist}
