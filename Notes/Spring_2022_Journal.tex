...
Hmm, looks like calculating V_inv takes a lot of time? Speed up using tcrossprod? or crossprod?
...
Oh, also only have to calculate G_old once.


Ok, think I have it sort of working. But it is not acting corretly I belieive. the G_new is so much larger than the G_original each time, so it requries $\gamma$ be almost 0, which means that the new beta is the same as the old one so then the algorithm basicaly fignishes running.
