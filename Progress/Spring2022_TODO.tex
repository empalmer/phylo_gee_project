...
\begin{itemize}
  ...
  \begin{todolist}
    ...
    \item[\done] Figure out why geem code uses + instead of - on update
    \begin{itemize}
      \item I think + is correct. At least it converges.
      \item From meeting: - is correct, but need a - in hessian so cancels out. So + if no - in hessian
    \end{itemize}
    \item[\wontfix] Identify which covariate is causing the trouble.
    \item[\wontfix] Try removing the single OTU that seems to be contributing to problem.
...
\begin{todolist}
  \item Include more taxa: more zero-inflation
  ...
  \item Wednesday email research update.
